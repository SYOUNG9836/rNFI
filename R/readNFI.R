...
readNFI <- function(dir,location=NULL){
  ...
  for(i in 1:length(filenames)){
    ...
    ##일반정보
    General_info <- readxl::read_excel(paste(dir, filenames[i], sep = ""), sheet="일반정보", col_names=TRUE, col_types = "text")
    ...
    ##비산림면적
    Non_forest <-readxl:: read_excel(paste(dir, filenames[i], sep = ""), sheet="비산림면적", col_names=TRUE, col_types = "text")
    N_cname <- c("기본조사원 비산림면적", "대경목조사원 비산림면적")
    Non_forest[ , N_cname ] <- lapply ( Non_forest[ , N_cname ], as.numeric)
    
    
    ##"임분조사표"
    Stand_inve <- readxl::read_excel(paste(dir, filenames[i], sep = ""), sheet="임분조사표", col_names=TRUE, col_types = "text")
    ...
    ##"임목조사표"
    Tree_inve <- readxl::read_excel(paste(dir, filenames[i], sep = ""), sheet="임목조사표", col_names=TRUE, col_types = "text")
